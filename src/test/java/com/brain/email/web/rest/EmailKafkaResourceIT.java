...
import com.brain.email.config.KafkaProperties;
...
class EmailKafkaResourceIT {
    ...
    void setup() {
        ...
        EmailKafkaResource kafkaResource = new EmailKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/email-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/email-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
